  ...
  static async updateProduct(req, res) {
    ...
    try {
      if (productUpdate) {
        return res
          .status(200)
          .json(response("Success", "Sukses update data produk!", showProduct));
      } else {
        return res
          .status(400)
          .json(response("Failed!", "Data data produk tidak ada!", "Kosong"));
      }
    } catch (error) {
  ...
  static async deleteProduct(req, res) {
    ...
    try {
      if (delProduct) {
        return res
          .status(200)
          .json(response("Success", "Sukses hapus data produk!", `ID : ${id}`));
      } else {
        return res
          .status(400)
          .json(response("Failed", "Data produk tidak ada!", `ID : ${id}`));
      }
    } catch (error) {
      return res.status(400).json(response("Failed", error.message, "Kosong"));
    }
